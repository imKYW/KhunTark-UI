...
-- Spawn --------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------
oUF:RegisterStyle("CombaUI - NP", function(frame, unit)
    if unit == "player" then
    ...
    elseif unit:match("nameplate") then
        ...
        bg:SetStatusBarTexture(cfg.texture)
		bg.multiplier = 0.4
    end
end)
oUF:SpawnNamePlates("CombaUI_NP", nil, npCVars)
--oUF:SpawnNamePlates(name, nil, npCVars)
